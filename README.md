...
| 源程序量 | 120 个源程序文件，共 15298 行（Java 源程序 5299 行、MyBatis 映射文件 943 行、前端页面 7318 行、系统配置 44 行、数据库脚本 1261 行、单元测试 433 行），按每页 50 行排版共 306 页；使用源程序文档生成工具生成的源程序文档（含文件标题行与页眉）共 315 页 |
...
| 我的档案 | 志愿者 | 维护服务技能标签与常住定位，作为供需匹配依据 |
...
| 活动结算与信用分 | 志愿者组织 | 活动结束后结算考勤，记录爽约并扣减信用分；服务时长复核通过后累加累计时长并奖励信用分 |
...
- **业务层**：`service` / `service.impl` 分层接口与实现，业务实现基于 MyBatis-Plus `ServiceImpl` 提供标准增删改查能力，
  并实现活动申报、活动审核、重新申报、可信签到、供需匹配、报名候补与活动结算等业务方法。
- **数据访问层**：MyBatis-Plus `BaseMapper` + 自定义映射文件 `resources/mapper/*.xml`；
  报名、活动审核、活动申报、签到签退、时长复核、活动结算等涉及多表写库或需要行锁校验的业务通过 MySQL 存储过程实现，保证业务数据的一致性。
- **安全控制**：Spring Security 实现登录认证与基于角色的页面、接口访问控制；密码以 PBKDF2 加盐摘要方式存储与校验，
  历史 MD5 摘要密文在账号登录成功后自动升级为加盐密文。
...
- **数据模型**：`volunteer` 志愿者表、`volunteer_skill` 志愿者技能表、`organization` 志愿者组织表、`admin` 平台管理员表、
  ...
  `checkin` 志愿服务签到表、`training` 志愿培训表、`volunteer_training_situation` 志愿者培训情况表、
  `act_announcement` 志愿活动公告表、`policy_announcement` 通知公告表、`policy_file` 公告附件表、
  `show` 志愿秀表、`show_picture` 志愿秀图片表。
...
| 我的报名（签到签退） | http://localhost:8111/volunteer/my_participation.html |
...
| 材料 | 说明 |
| --- | --- |
| 源程序文档 | 已生成 [`docs/源程序.txt`](docs/源程序.txt)（120 个源程序文件、15742 行，每页 50 行共 315 页）；源程序变动后可执行 `python3 tools/source_doc.py --output docs/源程序.txt` 重新生成，按“前后各连续 30 页”提交时追加 `--head-tail 30` |
