...
| 源程序量 | 187 个源程序文件，共 32199 行（Java 源程序 12092 行、MyBatis 映射文件 1964 行、前端页面 15619 行、系统配置 52 行、数据库脚本 1898 行、单元测试 574 行），按每页 50 行排版共 644 页；使用源程序文档生成工具生成的源程序文档（含文件标题行与页眉）共 659 页 |
...
- **定时任务**：`PlatformTaskScheduler` 周期执行，按活动时间自动流转活动状态，并释放活动开始前仍未确认参加的报名名额。
...
- **数据模型**：`volunteer` 志愿者表、`volunteer_skill` 志愿者技能表、`organization` 志愿者组织表、`admin` 平台管理员表、
  ...
  `notification` 站内通知表、`audit_log` 操作审计表。
...
| 材料 | 说明 |
| --- | --- |
| 源程序文档 | 已生成 [`docs/源程序.txt`](docs/源程序.txt)（187 个源程序文件、32935 行，每页 50 行共 659 页）；源程序变动后可执行 `python3 tools/source_doc.py --output docs/源程序.txt` 重新生成，按“前后各连续 30 页”提交时追加 `--head-tail 30` |
